...
class DumpFileFixer:
    def add_subfolder(self, output_file, subfolder_name):
        """Add 'Alarm Text Output' subfolder before the file name"""
        output_dir = os.path.dirname(output_file)
        base_name = os.path.splitext(os.path.basename(output_file))[0] + '_alarms.txt'
        alarm_folder = os.path.join(output_dir, subfolder_name)
        os.makedirs(alarm_folder, exist_ok=True)
        return os.path.join(alarm_folder, base_name)

    def remove_duplicate_alarms(self):
        """Remove duplicate alarms from the alarm points"""
        unique_alarms = []
        seen = set()
        for point_info in self.alarm_points:
            key = (point_info.get('name'), point_info.get('type_info'))
            if key not in seen:
                unique_alarms.append(point_info)
                seen.add(key)
        self.alarm_points = unique_alarms
        self.debug_print(f"Total alarms after deduplication: {len(self.alarm_points)}")

    def find_matching_alarm(self, point_name):
        """Find a matching alarm point by name"""
        isHigh = 'Hi' in point_name
        # Try replacing 'High' with 'Low'
        new_point_name = point_name.replace('High', 'Low')
        # If no change, try replacing 'Hi' with 'Low'
        if new_point_name == point_name:
            new_point_name = point_name.replace('Hi', 'Low')
        point_name = new_point_name
        for point_info in self.alarm_points:
            if point_info['name'] == point_name:
                limit = point_info.get('Limit', 0)
                normal = point_info.get('Normal', 0)
                try:
                    limit_val = float(limit) if limit not in (None, '') else 0.0
                except (ValueError, TypeError):
                    limit_val = 0.0
                try:
                    normal_val = float(normal) if normal not in (None, '') else 0.0
                except (ValueError, TypeError):
                    normal_val = 0.0
                if isHigh:
                    SP = min(limit_val, normal_val)
                else:
                    SP = max(limit_val, normal_val)
                self.debug_print(f"Found Match for {point_name}: isHigh={isHigh}, SP={SP}")
                # Remove 'name' from point_info to avoid duplication in CSV
                # del self.alarm_points['name']
                return isHigh, SP
        return None
    def alarm_to_CSV_ECO(self, output_file):
        """Export alarm points to a CSV file"""
        report_file = self.add_subfolder(output_file,"Alarm CSV Output\EcoStruxure")
        alarm_rows = []
        for point_info in self.alarm_points:
            converted_alarm = {}
            # Ensure type_info is int for comparison
            type_info = point_info.get('type_info')
            try:
                type_info = int(type_info)
            except (ValueError, TypeError):
                type_info = None
            if type_info == 337:
                # Limit may be string, so compare as float
                try:
                    limit_val = float(point_info.get('Limit', 0))
                except (ValueError, TypeError):
                    limit_val = 0.0
                if limit_val == 1:
                    converted_alarm['TYPE'] = "alarm.ChangeOfStateAlarm"
                    point_info['LowLimit'] = 0 
                    point_info['HighLimit'] = 0
                else:
                    converted_alarm['TYPE'] = "alarm.OutOfRangeAlarm"
                    temp = self.find_matching_alarm(point_info['name'])
                    #self.Dup_Count += 0
                    if temp is not None:
                        isHigh, SP = temp
                        if isHigh:
                            converted_alarm['LowLimit'] = SP
                            converted_alarm['HighLimit'] = point_info.get('Normal', '')
                        else:
                            converted_alarm['HighLimit'] = SP
                            converted_alarm['LowLimit'] = point_info.get('Normal', '')
            elif type_info == 340:
                converted_alarm['TYPE'] = "alarm.ChangeOfStateAlarm"
            else:
                # Default case for unknown types
                converted_alarm['TYPE'] = "alarm.UnknownAlarmType"
            converted_alarm['NAME'] = point_info.get('name', '')
            converted_alarm['DESCR'] = point_info.get('description', '') if 'description' in point_info else ''
            converted_alarm['NOTE1'] = point_info.get('TYPE', '')
            converted_alarm['NOTE2'] = point_info.get('EventType', '')
            converted_alarm['AlarmState'] = "Reset"
            converted_alarm['MonitoredVariable'] = "..\\..\\" + point_info.get('controller', '') + "\\" + point_info.get('name', '') + "\\Value"
            if "Enabled" in point_info.get('alarms', []):
                converted_alarm['StateValue'] = "TRUE"
            else:
                converted_alarm['StateValue'] = ""
            converted_alarm['TimeDelayToNormal'] = point_info.get('timeDelay', '')
            converted_alarm['AlarmMessage'] = point_info.get('MsgTextOffNormal', '')
            converted_alarm['ResetMessage'] = point_info.get('MsgTextNormal', '')
            converted_alarm['AlarmPriority'] = 200
            converted_alarm['ResetPriority'] = 200
            # Ensure numeric values for subtraction
            limit = point_info.get('Limit', '')
            normal = point_info.get('Normal', '')
            try:
                limit_val = float(limit) if limit not in (None, '') else 0.0
            except (ValueError, TypeError):
                limit_val = 0.0
            try:
                ...
                normal_val = 0.0
            # Only set HighLimit/LowLimit if not already set above
            if 'HighLimit' not in converted_alarm:
                converted_alarm['HighLimit'] = limit
            if 'LowLimit' not in converted_alarm:
                converted_alarm['LowLimit'] = normal
            converted_alarm['Deadband'] = abs(limit_val - normal_val)
            alarm_rows.append(converted_alarm)
        with open(report_file, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = [
                'TYPE', 'NAME', 'DESCR', 'NOTE1', 'NOTE2', 'AlarmState',
                'MonitoredVariable', 'StateValue', 'TimeDelayToNormal',
                'AlarmMessage', 'ResetMessage', 'AlarmPriority', 'ResetPriority',
            ...
            for row in alarm_rows:
                writer.writerow(row)         
    def alarm_to_CSV_CON(self, output_file):
        """Export alarm points to a CSV file"""
        report_file = self.add_subfolder(output_file,"Alarm CSV Output\Continuum")
        alarm_rows = []
        for point_info in self.alarm_points:
            converted_alarm = {}

            event_type = point_info.get('EventType', 'Unknown')
            # Set TYPE based on event_type
            if event_type.startswith("ACC"):
                event_type_trimmed = event_type[3:]
                converted_alarm['TYPE'] = "infinity." + event_type_trimmed
            elif event_type.startswith("Hi"):
                converted_alarm['TYPE'] = "infinity.HighLimit"
            else:
                converted_alarm['TYPE'] = "infinity.LowLimit"
            
            # Extract the name after the last backslash and before the first space (if any)
            alarms_field = point_info.get('alarms', [])
            if alarms_field and isinstance(alarms_field, list) and alarms_field[0]:
                alarm_str = alarms_field[0]
                # Remove anything before and including the last backslash
                name_part = alarm_str.split("\\")[-1]
                # Remove everything after the first space
                name = name_part.split(" ")[0]
            else:
                name = ""
            converted_alarm['NAME'] = name
            converted_alarm['DESCR'] = point_info.get('description', '') if 'description' in point_info else ''
            converted_alarm['TimeDelay'] = point_info.get('timeDelay', '') + ' s'
            converted_alarm["Expression"] = point_info.get('ACCExpression', '')
            converted_alarm['ResetMessage'] = point_info.get('MsgTextNormal', '')
            converted_alarm['AlarmPriority'] = 200
            converted_alarm['ResetPriority'] = 200

            if "Hi" in point_info.get('EventType', ''):
                try:
                    limit_val = int(point_info.get('Limit', 0))
                except (ValueError, TypeError):
                    limit_val = 0.0
                try:
                    normal_val = int(point_info.get('Normal', 0))
                except (ValueError, TypeError):
                    normal_val = 0.0

                if limit_val or normal_val:
                    limit_str = f"{limit_val:.2f}"
                    normal_str = f"{normal_val:.2f}"
                else:
                    limit_str = point_info.get('Limit', '')
                    normal_str = point_info.get('Normal', '')

                converted_alarm['HighLimit'] = limit_str
                converted_alarm['ReturnToNormal'] = normal_str
                converted_alarm['LowLimit'] = None
            else:
                try:
                    limit_val = int(point_info.get('Limit', 0))
                except (ValueError, TypeError):
                ...
                converted_alarm['HighLimit'] = None
                converted_alarm['ReturnToNormal'] = normal_str
                converted_alarm['LowLimit'] = limit_str

            converted_alarm['AlarmMessage'] = point_info.get('MsgTextOffNormal', '')

            alarm_rows.append(converted_alarm)

            # Remove duplicate alarms with matching TYPE and NAME
        self.remove_duplicate_alarms()
        
        # Write the alarm rows to a CSV file
            
        with open(report_file, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = [
                'TYPE', 'NAME', 'DESCR', 'TimeDelay', 'Expression',
                'ResetMessage', 'AlarmPriority', 'ResetPriority',
                'HighLimit', 'LowLimit', 'ReturnToNormal', 'AlarmMessage'
            ]
    ...
    def write_alarm_report(self, output_file):
        ...
        self.alarm_to_CSV_ECO(output_file)
        self.alarm_to_CSV_CON(output_file)
        report_file = self.add_subfolder(output_file, "Alarm Text Output")
        try:
            ...
            messagebox.showinfo("Alarm Report", f"Alarm report saved as: {report_file}")
            self.debug_print("Alarm report written to CSV as well.")
            return True
    ...
    def run(self):
        ...
        if AutoLoadInput:
            input_file = 'G:\Yscripts\Richmar Stuff\Daryl\Dumpfiles\Yonkers Raceway 04_2025\YonkersRaceway.dmp'
            scan_file = 'G:\Yscripts\Richmar Stuff\Daryl\Dumpfiles\Yonkers Raceway 04_2025\Root.dmp'
            output_file = 'G:\Yscripts\Richmar Stuff\Daryl\DumpFileFix and Alarm Extractor\Workspace\Test Script Outputs\DFFAE_Output.txt'
        else:   
            # Select input file
        
            input_file = filedialog.askopenfilename(
                title="Select Network Dump File",
            ...
            output_file = filedialog.asksaveasfilename(
                title="Save Converted Files A File As",
        ...
        try:
            ...
            message = (
                ...
                #f"Duplicate Alarms Deleted:{Dup_Count}\n"
            )
